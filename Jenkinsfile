pipeline{
    ...
    stages{
        ...
        stage('push image to DockerHub'){
            steps{
                script{
                    withCredentials([string(credentialsId: 'dockercreds', variable: 'docker_hub_cred')]) {

                    sh 'sudo docker login -u naren4897 -p ${docker_hub_cred}'
                    sh 'sudo docker image push naren4897/$JOB_NAME:v1.$BUILD_ID'
                    sh 'sudo docker image push naren4897/$JOB_NAME:latest'
                    }
